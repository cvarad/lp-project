import googlemaps
gmaps = googlemaps.Client(key='AIzaSyCIPHtFEFHbQ4tG7ZYQEYtPg6UHJ1rn5r4')


def generate_distance_matrix(addresses):
    distance_matrix_response = gmaps.distance_matrix(origins=addresses,destinations=addresses,mode='driving',language='json',units='metric')
    distance_matrix = []
    distance_matrix = [[row['elements'][j]['distance']['value'] for j in range(len(row['elements']))] for row in distance_matrix_response['rows']]
    duration_matrix = [[row['elements'][j]['duration']['value'] for j in range(len(row['elements']))] for row in distance_matrix_response['rows']]
